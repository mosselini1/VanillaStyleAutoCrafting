
/*
 *    MCreator note: This file will be REGENERATED on each build.
 */
package net.mcreator.vsac.init;

import net.minecraftforge.registries.RegistryObject;
import net.minecraftforge.registries.ForgeRegistries;
import net.minecraftforge.registries.DeferredRegister;

import net.minecraft.world.level.block.entity.BlockEntityType;
import net.minecraft.world.level.block.Block;

import net.mcreator.vsac.block.entity.AutoCrafterBlockEntity;
import net.mcreator.vsac.VanillastyleautocraftingMod;

public class VanillastyleautocraftingModBlockEntities {
	public static final DeferredRegister<BlockEntityType<?>> REGISTRY = DeferredRegister.create(ForgeRegistries.BLOCK_ENTITY_TYPES,
			VanillastyleautocraftingMod.MODID);
	public static final RegistryObject<BlockEntityType<?>> AUTO_CRAFTER = register("auto_crafter", VanillastyleautocraftingModBlocks.AUTO_CRAFTER,
			AutoCrafterBlockEntity::new);

	private static RegistryObject<BlockEntityType<?>> register(String registryname, RegistryObject<Block> block,
			BlockEntityType.BlockEntitySupplier<?> supplier) {
		return REGISTRY.register(registryname, () -> BlockEntityType.Builder.of(supplier, block.get()).build(null));
	}
}
